...
- Better decompiler for LUA 5.1, source code generated by unluac hard to follow
...
Large parts of Lightroom, including all the modules (Library, Develop, Location, etc.), are written in LUA 5.1. With the exception `AgKernel`, the LUA code is only available as byte code. 
...
I use [Resource Hacker](https://angusj.com/resourcehacker/) to extract the LUA files (Windows only), [unluac](https://sourceforge.net/projects/unluac/) to decompile the LUA binaries to somewhat human-readable source code, and [Ghidra]() to decompile native code.

## Location module

The LUA libraries for Lightroom's Location module are stored in the file `Location.lrmodule`.

The Location module uses [Google Maps Javascript API](https://developers.google.com/maps/documentation/javascript). Specifically, Lightroom Classic 6.14 was developed using Google Maps JS API v3.12.
...
LUA code to generate HTML for the map view is embedded in `LOCATONMAPVIEW.LUA`. The resources of `Location.lrmodule` also contain images and a few `.JS` files that are referenced in the Javascript code generated by `LOCATIONMAPVIEW.LUA`. 
...
Javascript running in the map calls back to LUA through `hosteval()`. See `callCallback = function()`. Additional Javascript code is directly invoked from LUA across several libraries.
...
Under the hood, the map view uses the [Chromium Embedded Framework](https://github.com/chromiumembedded/cef) (CEF). Native code and related LUA libraries are stored in the file `cef_toolkit.dll`.
...
Replacing `google-map` with an arbirtrary string returns the same result, indicating that this the service status functioanlity is no longer active. Though still worth documenting in case of issues when the URL goes offline or Adobe changes its mind.

The LUA code responsible is located in `AGSERVICESTATUS.LUA` found in `LightroomSDK.dll`.
...
Lightroom tries to access the following files:
```
...
`devtools_resources.pak` contains JavaScript, developer console??
